...
//Below line updated by NIkita jethava
import com.google.gerrit.server.git.meta.MetaDataUpdate;
//below line is updated by Nikita jethava
...
public class PutPmsUrl implements RestModifyView<ProjectResource, PmsUrlInput> {
  ...
  @Inject
  PutPmsUrl(ProjectCache cache, MetaDataUpdate.Server updateFactory) {
    this.cache = cache;
  ...
  public Response<String> apply(ProjectResource resource, PmsUrlInput input)
    ...
    if (input == null) {
      input = new PmsUrlInput(); // Delete would set description to null.
    }
    ...
    try (MetaDataUpdate md = updateFactory.create(resource.getNameKey())) {
      ...
      project.setPmsUrl(Strings.emptyToNull(input.pmsUrl));
      String msg =
          MoreObjects.firstNonNull(
              Strings.emptyToNull(input.commitMessage), "Updated PMS URL.\n");
      if (!msg.endsWith("\n")) {
        msg += "\n";
      }
      md.setAuthor(user);
